                 ...
                 const char *device)
{
    ...
    return (props && !props->executed[op->phase]
           && !props->disallowed[op->phase])
           && !(pcmk__str_eq(op->action, "on", pcmk__str_casei) && !pcmk_is_set(props->device_support_flags, st_device_supports_on)) ? props : NULL;
}
                 ...
                 const char *device, gboolean verified_devices_only)
{
    ...
    crm_trace("Removing %s from %s (%d remaining)",
              device, peer->host, count_peer_devices(op, peer, FALSE, FALSE));
        ...
        if (pcmk_is_set(op->call_options, st_opt_topology)) {
            ...
            if (grab_peer_device(op, peer, device, verified_devices_only)) {
                return peer;
                   ...
                   && count_peer_devices(op, peer, verified_devices_only, pcmk__str_eq(op->action, "on", pcmk__str_casei))) {
