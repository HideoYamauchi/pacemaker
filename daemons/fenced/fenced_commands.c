...
/*
In the case of "dynamic-list", the process is executed based on the list result, so dev-> targets is unnecessary.
*/
    value = target_list_type(device);
    if (pcmk__str_eq(value, "dynamic-list", pcmk__str_casei) && device->targets) {
        g_list_free_full(device->targets, free);
        device->targets = NULL;
    }
...
/*
        Not needed because dev-> targets is already a null block.
