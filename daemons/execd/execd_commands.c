        ...
        if (pcmk__result_ok(&(cmd->result))
            ...
            if (cmd->result.execution_status == PCMK_EXEC_PENDING) {
                ...
                if ((cmd->result.execution_status == PCMK_EXEC_DONE)
                  ...
                  (cmd->interval_ms > 0)) { 
            /* For monitors other than follow up monitors, the pending state continues from           */
            /* the time of the first notification until the timeout, it will be treated as a timeout. */
            if ((cmd->result.execution_status == PCMK_EXEC_PENDING) &&
